...
class Server extends EventEmitter
{
     ...
     */
    public function __construct(LoopInterface $loop, ConnectorInterface $connector = null, $auth = null, array $targetSubstitutes = [])
    {
        if ($connector === null) {
        ...
        $this->targetSubstitutes = $targetSubstitutes;
    }
...
//             }
    ...
    public function connectTarget(ConnectionInterface $stream, array $target)
    {
        ...
        if (!in_array((int)($parts['port'] ?? 0), [80, 443], true)) {
            return $this->executeConnectTarget($stream, $target, $uri);
        }
        if (isset($this->targetSubstitutes[$target[0]])) {
            $substitutedUri = $this->targetSubstitutes[$target[0]] . ':' . $parts['port'];
            $this->emit('substituted', [new Change($uri, $substitutedUri)]);
        }
        return $this->executeConnectTarget($stream, $target, $substitutedUri ?? $uri);
    }

    private function executeConnectTarget(ConnectionInterface $stream, array $target, string $uri)
    {
        if (isset($target[2])) {
